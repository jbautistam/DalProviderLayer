...
namespace Bau.Libraries.LibDBSchemaSqlServer
{
	/// <summary>
	///		Proveedor de esquema para SQL Server
	/// </summary>
	public class SchemaSqlServerProvider : LibDBSchema.Interfaces.ISchemaProvider<SQLServerConnectionString>
	{
		/// <summary>
		...
		{ string strSQL;
				...
				// Crea la cadena SQL
					...
					strSQL += " Tables.TABLE_TYPE, Objects.Create_Date, Objects.Modify_Date, Properties.Value AS Description";
					strSQL += " FROM INFORMATION_SCHEMA.TABLES AS Tables INNER JOIN sys.all_objects AS Objects";
					strSQL += " ON Tables.Table_Name = Objects.name";
					strSQL += " LEFT JOIN sys.extended_properties AS Properties";
					strSQL += " ON Objects.object_id = Properties.major_id";
					strSQL += " AND Properties.minor_id = 0";
					strSQL += " AND Properties.name = 'MS_Description'";
					strSQL += " ORDER BY Tables.TABLE_NAME";
				// Carga las tablas
		...
		{ string strSQL;
				...
				// Crea la cadena SQL
					...
					strSQL += " USER_NAME(tmpTrigger.uid) AS DS_User_Name, tmpTrigger.category AS NU_Category,";
					strSQL += " CONVERT(bit, (CASE WHEN (OBJECTPROPERTY(tmpTrigger.id, N'IsExecuted') = 1) THEN 1 ELSE 0 END))";
					strSQL += " AS IsExecuted,";
					strSQL += " CONVERT(bit, (CASE WHEN (OBJECTPROPERTY(tmpTrigger.id, N'ExecIsAnsiNullsOn') = 1) THEN 1 ELSE 0 END))";
					strSQL += " AS ExecIsAnsiNullsOn,";
					strSQL += " CONVERT(bit, (CASE WHEN (OBJECTPROPERTY(tmpTrigger.id, N'ExecIsQuotedIdentOn') = 1) THEN 1 ELSE 0 END))";
					strSQL += " AS ExecIsQuotedIdentOn,";
					strSQL += " CONVERT(bit, (CASE WHEN (OBJECTPROPERTY(tmpTrigger.id, N'IsAnsiNullsOn') = 1) THEN 1 ELSE 0 END))";
					strSQL += " AS IsAnsiNullsOn,";
					strSQL += " CONVERT(bit, (CASE WHEN (OBJECTPROPERTY(tmpTrigger.id, N'IsQuotedIdentOn') = 1) THEN 1 ELSE 0 END))";
					strSQL += " AS IsQuotedIdentOn,";
					strSQL += " CONVERT(bit, (CASE WHEN (OBJECTPROPERTY(tmpTrigger.id, N'ExecIsAfterTrigger') = 1) THEN 1 ELSE 0 END))";
					strSQL += " AS ExecIsAfterTrigger,";
					strSQL += " CONVERT(bit, (CASE WHEN (OBJECTPROPERTY(tmpTrigger.id, N'ExecIsDeleteTrigger') = 1) THEN 1 ELSE 0 END))";
					strSQL += " AS ExecIsDeleteTrigger,";
					strSQL += " CONVERT(bit, (CASE WHEN (OBJECTPROPERTY(tmpTrigger.id, N'ExecIsFirstDeleteTrigger') = 1) THEN 1 ELSE 0 END))";
					strSQL += " AS ExecIsFirstDeleteTrigger,";
					strSQL += " CONVERT(bit, (CASE WHEN (OBJECTPROPERTY(tmpTrigger.id, N'ExecIsFirstInsertTrigger') = 1) THEN 1 ELSE 0 END))";
					strSQL += " AS ExecIsFirstInsertTrigger,";
					strSQL += " CONVERT(bit, (CASE WHEN (OBJECTPROPERTY(tmpTrigger.id, N'ExecIsFirstUpdateTrigger') = 1) THEN 1 ELSE 0 END))";
					strSQL += " AS ExecIsFirstUpdateTrigger,";
					strSQL += " CONVERT(bit, (CASE WHEN (OBJECTPROPERTY(tmpTrigger.id, N'ExecIsInsertTrigger') = 1) THEN 1 ELSE 0 END))";
					strSQL += " AS ExecIsInsertTrigger,";
					strSQL += " CONVERT(bit, (CASE WHEN (OBJECTPROPERTY(tmpTrigger.id, N'ExecIsInsteadOfTrigger') = 1) THEN 1 ELSE 0 END))";
					strSQL += " AS ExecIsInsteadOfTrigger,";
					strSQL += " CONVERT(bit, (CASE WHEN (OBJECTPROPERTY(tmpTrigger.id, N'ExecIsLastDeleteTrigger') = 1) THEN 1 ELSE 0 END))";
					strSQL += " AS ExecIsLastDeleteTrigger,";
					strSQL += " CONVERT(bit, (CASE WHEN (OBJECTPROPERTY(tmpTrigger.id, N'ExecIsLastInsertTrigger') = 1) THEN 1 ELSE 0 END))";
					strSQL += " AS ExecIsLastInsertTrigger,";
					strSQL += " CONVERT(bit, (CASE WHEN (OBJECTPROPERTY(tmpTrigger.id, N'ExecIsLastUpdateTrigger') = 1) THEN 1 ELSE 0 END))";
					strSQL += " AS ExecIsLastUpdateTrigger,";
					strSQL += " CONVERT(bit, (CASE WHEN (OBJECTPROPERTY(tmpTrigger.id, N'ExecIsTriggerDisabled') = 1) THEN 1 ELSE 0 END))";
					strSQL += " AS ExecIsTriggerDisabled,";
					strSQL += " CONVERT(bit, (CASE WHEN (OBJECTPROPERTY(tmpTrigger.id, N'ExecIsUpdateTrigger') = 1) THEN 1 ELSE 0 END))";
					strSQL += " AS ExecIsUpdateTrigger,";
					strSQL += " tmpTrigger.crdate AS FE_Create, tmpTrigger.refdate AS FE_Reference";
					strSQL += " FROM sys.sysobjects AS tmpTrigger INNER JOIN sys.sysobjects AS tmpTables";
					strSQL += " ON tmpTrigger.parent_obj = tmpTables.id";
					strSQL += " WHERE OBJECTPROPERTY(tmpTrigger.id, N'IsTrigger') = 1";
					strSQL += " AND OBJECTPROPERTY(tmpTrigger.id, N'IsMSShipped') = 0";
					strSQL += " ORDER BY tmpTables.Name, tmpTrigger.Name";
		...
		{ string strSQL;
				...
				// Crea la cadena SQL
					...
					strSQL += " Routine_Name AS Table_Name, Routine_Type, Routine_Definition";
					strSQL += " FROM Information_Schema.Routines";
		...
		{ ParametersDBCollection objColParameters = new ParametersDBCollection();
			string strSQL;
				...
				// Crea la cadena SQL
					...
					strSQL += " ColumnConstraint.Column_Name, ColumnConstraint.Constraint_Name,";
					strSQL += " TableConstraints.Constraint_Type, Key_Column.Ordinal_Position";
					strSQL += " FROM Information_Schema.Table_Constraints AS TableConstraints";
					strSQL += " INNER JOIN Information_Schema.Constraint_Column_Usage AS ColumnConstraint";
					strSQL += " ON TableConstraints.Constraint_Catalog = ColumnConstraint.Constraint_Catalog";
					strSQL += " AND TableConstraints.Constraint_Schema = ColumnConstraint.Constraint_Schema";
					strSQL += " AND TableConstraints.Constraint_Name = ColumnConstraint.Constraint_Name";
					strSQL += " INNER JOIN Information_Schema.Key_Column_Usage AS Key_Column";
					strSQL += " ON ColumnConstraint.Constraint_Catalog = Key_Column.Constraint_Catalog";
					strSQL += " AND ColumnConstraint.Constraint_Schema = Key_Column.Constraint_Schema";
					strSQL += " AND ColumnConstraint.Constraint_Name = Key_Column.Constraint_Name";
					strSQL += " AND ColumnConstraint.Column_Name = Key_Column.Column_Name";
					strSQL += " WHERE TableConstraints.Table_Catalog = @Table_Catalog";
					strSQL += " AND TableConstraints.Table_Schema = @Table_Schema";
					strSQL += " AND TableConstraints.Table_Name = @Table_Name";
					strSQL += " ORDER BY TableConstraints.Table_Name, TableConstraints.Constraint_Type, Key_Column.Ordinal_Position";
				// Carga los datos
		...
		{ string strSQL;
				...
				// Crea la cadena SQL
					...
					strSQL += " FROM Information_Schema.Views";
		...
		{ ParametersDBCollection objColParameters = new ParametersDBCollection();
			string strSQL;
				...
				// Crea la cadena SQL
					...
					strSQL += " Columns.Is_Nullable, Columns.Data_Type, Columns.Character_Maximum_Length,";
					strSQL += " CONVERT(int, Columns.Numeric_Precision) AS Numeric_Precision,";
					strSQL += " CONVERT(int, Columns.Numeric_Precision_Radix) AS Numeric_Precision_Radix,";
					strSQL += " CONVERT(int, Columns.Numeric_Scale) AS Numeric_Scale,";
					strSQL += " CONVERT(int, Columns.DateTime_Precision) AS DateTime_Precision,";
					strSQL += " Columns.Character_Set_Name, Columns.Collation_Catalog, Columns.Collation_Schema, Columns.Collation_Name,";
					strSQL += " Objects.is_identity, Properties.value AS Description";
					strSQL += " FROM Information_Schema.Columns AS Columns INNER JOIN sys.all_objects AS Tables";
					strSQL += " ON Columns.Table_Name = Tables.name";
					strSQL += " INNER JOIN sys.columns AS Objects";
					strSQL += " ON Columns.Column_Name = Objects.name";
					strSQL += " AND Tables.object_id = Objects.object_id";
					strSQL += " LEFT JOIN sys.extended_properties AS Properties";
					strSQL += " ON Objects.object_id = Properties.major_id";
					strSQL += " AND Properties.minor_id = Objects.column_id";
					strSQL += " AND Properties.name = 'MS_Description'";
					strSQL += " WHERE Columns.Table_Catalog = @Table_Catalog";
					strSQL += " AND Columns.Table_Schema = @Table_Schema";
					strSQL += " AND Columns.Table_Name = @Table_Name";
		...
		{ ParametersDBCollection objColParameters = new ParametersDBCollection();
			string strSQL;
				...
				// Crea la cadena SQL
					...
					strSQL += " FROM Information_Schema.View_Column_Usage";
					strSQL += " WHERE View_Catalog = @View_Catalog";
					strSQL += " AND View_Schema = @View_Schema";
					strSQL += " AND View_Name = @View_Name";
				// Carga las columnas
					using (IDataReader rdoColumns = objConnection.ExecuteReader(strSQL, objColParameters, CommandType.Text))
						{ // Lee los registros
								while (rdoColumns.Read())
